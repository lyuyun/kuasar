...
The transport is a Unix domain socket (`SOCK_STREAM`, default `/run/warmfork-readiness.sock`). The full protocol (both modes), multi-container two-phase barrier, REJECT/CANCEL rollback semantics, pod annotations, and workload adaptation guide are specified in [WarmFork Readiness and Injection Protocol](warmfork_readiness_and_injection_protocol.md).
