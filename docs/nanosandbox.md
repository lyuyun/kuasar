...
1. **极速冷启动**：通过微型内核、精简 initrd、懒加载镜像、设备模型最小化等手段，将 MicroVM 冷启动时间压缩到 100ms 以内。
...
- **VM 模板（VM Template）**：预先启动一批处于"刚完成内核引导、等待任务"状态的 MicroVM 实例（即 standby pool），当新请求到来时，从池中 clone 一个实例，注入容器配置后立即启动容器进程。此机制可将 Hypervisor+内核引导耗时完全从关键路径中移除。
- **直接 fork VM 进程**：对支持 fork 的 Hypervisor（如 Firecracker 的 `jailer`），复用已初始化的 Hypervisor 进程状态，避免重新加载 VMM 二进制和设备初始化。
...
- 内核与 rootfs 合并打包为单一 **initrd**（压缩后 < 8MB），通过 virtio-blk 直接加载，无需单独的 bootloader。
...
- VM 的 `/init`（即 initrd 入口）**直接 exec 为容器的 entrypoint 进程**，该进程成为 VM 内的 PID 1。
...
2. Manager 通过 Hypervisor API 暂停 VM（freeze vCPU）
3. 保存 CPU 状态到 snapshot.cpu
4. 扫描内存脏页位图，dump 内存到 snapshot.mem
   （对于 Base Snapshot，需 dump 完整内存；增量时仅 dump 自上次以来的脏页）
5. 保存设备状态到 snapshot.dev
6. 对文件系统可写层执行 COW 快照，记录 snapshot.fs_diff
7. 恢复 VM 运行（unfreeze vCPU），整个捕获过程对容器进程透明
8. 将快照元数据写入 Snapshot Store
```
...
- 支持**父子快照链**（类似 git commit），子快照只存储相对父快照的增量脏页，大幅减少存储开销。
...
- **无 shim 单点**：传统架构中 shim 进程是一个单点——shim 崩溃会导致其管理的所有容器失控。NanoSandbox 消除了 shim，Manager 本身是无状态的（VM 状态在 Hypervisor 进程和快照 store 中），Manager 重启不影响运行中的 VM。
...
3. **异步优先**：长时操作（Create、Start、Stop）均返回操作 ID，支持轮询或事件订阅方式获取结果。
...
service SandboxService {
    ...
    // 对于冷启动：触发内核引导 → Zero-Init exec。
    // 对于快照恢复：等同于 StartFromSnapshot。
    // 对应 containerd Controller.Start。
    rpc StartSandbox(StartSandboxRequest) returns (StartSandboxResponse);

    // StopSandbox 停止沙箱（向 VM 发送关机信号或直接 poweroff）。
...
message SandboxSpec {
    ...
    // 可选：若使用快照启动，填写 snapshot_id。
    // 若此字段非空，StartSandbox 将走快照恢复路径。
    string snapshot_id = 7;
    // 扩展字段，用于传递 Hypervisor 实现特有的配置（类型 URL 由实现方定义）。
    google.protobuf.Any hypervisor_options = 8;
    // 可选：沙箱标签，用于过滤和查询。
...
Task API 负责沙箱内容器进程（Task）的生命周期管理。在 NanoSandbox 的 1:1 模型中，每个 Sandbox 只有一个 primary task（容器的 entrypoint 进程），但支持通过 `ExecTask` 在 VM 内发起额外的 exec（如调试场景）。
...
service TaskService {
    ...
    // ListPids 列出沙箱内所有进程 PID（从 guest 侧 /proc 读取）。
...
message StatsResponse {
    uint64 cpu_usage_ns    = 1;   // 累计 CPU 使用时间（纳秒）
    uint64 memory_usage_kb = 2;   // 当前内存使用（KB）
    uint64 rx_bytes        = 3;   // 网络接收字节数
    uint64 tx_bytes        = 4;   // 网络发送字节数
    uint64 read_bytes      = 5;   // 块设备读字节数
    uint64 write_bytes     = 6;   // 块设备写字节数
}

// ProcessSpec 复用自 Sandbox API（见上文）。
message ProcessSpec {
    string entrypoint     = 1;
    repeated string args  = 2;
    repeated string envs  = 3;
...
service SnapshotService {
    ...
    // 等价于：CreateSandbox（spec.snapshot_id = snapshot_id）+ StartSandbox。
...
message StartFromSnapshotRequest {
    ...
    // 新沙箱的网络配置（恢复时需重新配置网络，因 IP/MAC 可能需要变更）。
...
// NetworkConfig 复用自 Sandbox API（见上文）。
message NetworkConfig {
    string tap_name    = 1;
    string mac_address = 2;
    string ip_cidr     = 3;
...
|----------|-----------------|------|
| `RunPodSandbox` | `CreateSandbox` + `StartSandbox` | VM 引导至"内核就绪、等待 exec"中间态 |
...
| 快照启动端到端延迟 | ~30ms（仍需 StartTask 完成 exec）| ~5ms（直接 VMRESUME，无 exec 步骤）|
...
| overlayfs（可选）| 可写层 |
| cgroup v2（namespaces only）| 进程隔离 |
| 网络栈（minimal）| 容器网络 |
| KVM guest 支持 | 虚拟化 |
...
**宿主机侧气球（balloon）预分配**：Serverless 节点上维护一个内存气球池，当新 VM 创建时，从池中快速分配已初始化的内存页（已清零），避免运行时缺页填零的延迟。

**KSM（Kernel Same-page Merging）**：同一函数的多个实例（从相同 Base Snapshot 恢复）在内存中有大量相同页（只读内核代码段、共享库等），KSM 可自动合并这些页，大幅降低多实例场景的内存总开销。实测同一函数 100 个实例，KSM 可节省约 40%~60% 的内存。
...
4. **多容器 Pod 支持有限**：1:1 VM-per-Container 模型使得 Kubernetes multi-container Pod（sidecar 模式）需要多个 VM 或特殊处理，增加管理复杂度。
